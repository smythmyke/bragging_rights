...
/// Simple Bet Slip Widget for point-based betting (no odds)
/// Used for sports without odds: NBA, NHL, MLB, Soccer, Tennis
class SimpleBetSlipWidget extends StatefulWidget {
  final SimpleBetSlip betSlip;
  final Function(SimpleBet) onRemoveBet;
  final Function(SimpleBet, int) onConfidenceChanged;
  final VoidCallback onClear;
  final Function(SimpleBetSlip) onPlaceBet;
  final double userBalance;
  final int wagerAmount;
  final Function(int) onWagerChanged;

  const SimpleBetSlipWidget({
    Key? key,
  ...
  @override
  State<SimpleBetSlipWidget> createState() => _SimpleBetSlipWidgetState();
}

class _SimpleBetSlipWidgetState extends State<SimpleBetSlipWidget>
    with SingleTickerProviderStateMixin {
  late AnimationController _controller;
  late Animation<double> _slideAnimation;
  ...
  @override
  Widget build(BuildContext context) {
    final allBets = widget.betSlip.allBets;
      ...
      builder: (context, scrollController) {
        return GlassmorphismContainer(
          ...
          child: Column(
            children: [
              _buildHandle(),
              _buildHeader(allBets.length),
              Expanded(
                child: ListView(
                  controller: scrollController,
                  padding: const EdgeInsets.all(16),
                  children: [
                    ...allBets.map(_buildSimpleBetItem),
                    const SizedBox(height: 16),
                    _buildWagerSelector(),
                    const SizedBox(height: 16),
                    _buildSummary(),
  ...
  Widget _buildEmptyState() {
    return Container(
      ...
      child: GlassmorphismContainer(
        ...
        child: const Center(
          child: Text(
            'Your bet slip is empty - Pick a winner to start!',
            style: TextStyle(
              color: AppTheme.primaryCyan.withOpacity(0.7),
              fontSize: 14,
            ),
  ...
  Widget _buildHeader(int betCount) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      decoration: BoxDecoration(
        border: Border(
  ...
  Widget _buildSimpleBetItem(SimpleBet bet) {
    return Container(
      margin: const EdgeInsets.only(bottom: 8),
      child: GlassmorphismContainer(
        blur: 5,
          ...
          children: [
              ...
              children: [
                ...
                if (!bet.isRequired)
                  IconButton(
                    ...
                    onPressed: () => widget.onRemoveBet(bet),
            ...
            LabeledConfidenceSelector(
              ...
              onConfidenceChanged: (newConfidence) {
                widget.onConfidenceChanged(bet, newConfidence);
              },
              enabled: true,
              label: 'Confidence:',
              showMultiplier: true,
            ...
            Row(
              ...
              children: [
                ...
                Text(
                  'Potential: ${bet.potentialPoints.toStringAsFixed(1)} pts',
                  style: const TextStyle(
                    color: AppTheme.neonGreen,
                    fontSize: 12,
                    fontWeight: FontWeight.bold,
        ...
        children: [
          ...
          Wrap(
            ...
            children: [25, 50, 100, 200].map((amount) {
              final isSelected = widget.wagerAmount == amount;
              return GestureDetector(
                onTap: () => widget.onWagerChanged(amount),
                child: Container(
                  padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                  decoration: BoxDecoration(
                    color: isSelected
  ...
  Widget _buildSummary() {
    final totalPotentialPoints = widget.betSlip.totalPotentialPoints;
    final hasEnoughBalance = widget.userBalance >= widget.wagerAmount;
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              Text(
                '${totalPotentialPoints.toStringAsFixed(1)} pts',
                style: const TextStyle(
                  color: AppTheme.neonGreen,
                  fontWeight: FontWeight.bold,
                  fontSize: 18,
  ...
  void _placeBet() {
    widget.onPlaceBet(widget.betSlip);
    HapticFeedback.heavyImpact();
      ...
      SnackBar(
        content: Row(
          children: [
            ...
            Expanded(
              child: Text(
                'Bet${widget.betSlip.allBets.length > 1 ? 's' : ''} placed! Good luck!',
                style: const TextStyle(color: Colors.white),
              ),
            ),
          ],
