...
class PoolAutoGenerator {
  ...
  Future<void> generateGamePools({
    ...
    try {
      ...
      // FREEMIUM MODEL: ALWAYS generate simple pick pools for free tier
      // Check what odds are available for premium tier
      ...
      // ALWAYS generate simple pick pools (free tier)
      debugPrint('🆓 Generating simple pick pools for ${game.gameTitle} (free tier)');
      await _generateSimplePickPools(game);

      // If odds available, also generate premium pools (Phase 2 - premium tier)
      // For now, skip premium pools since Phase 2 not implemented yet
      if (hasAnyOdds) {
        debugPrint('💎 Odds available but premium tier not implemented yet');
        // TODO Phase 2: Generate premium odds-based pools here
      }
  ...
  /// Generate simple pick pools for games without odds (NBA exhibitions, etc.)
  Future<void> _generateSimplePickPools(GameModel game) async {
    try {
      ...
      for (final buyIn in quickPlayBuyIns) {
        ...
        // Build pool name with season label if available
        String poolName = 'Simple Pick - $buyIn BR';
        if (game.seasonLabel != null) {
          poolName = '${game.seasonLabel} $poolName';
        }
        ...
        final pool = Pool(
          ...
          metadata: {
            'autoGenerated': true,
            'scoringType': 'simplePick', // Key identifier for simple pick pools
            'requiresOdds': false, // No odds needed
            'allowsConfidence': true, // Enable confidence multiplier (1-5 stars)
            'seasonType': game.seasonType,
            'seasonLabel': game.seasonLabel,
            'exhibitionType': game.exhibitionType,
          },
        );

        final docRef = _firestore.collection('pools').doc();
        batch.set(docRef, pool.copyWith(id: docRef.id).toFirestore());
        debugPrint('   Created simple pick pool: $poolName');
      }

      await batch.commit();
      debugPrint('✅ Generated ${quickPlayBuyIns.length} simple pick pools for ${game.gameTitle}');
    } catch (e) {
      debugPrint('❌ Error generating simple pick pools: $e');
    }
  }
}
